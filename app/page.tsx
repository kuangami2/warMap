...
import { EventComparePanel } from '@/components/EventComparePanel';
...
export default function Home() {
  ...
  const [comparisonWar, setComparisonWar] = useState<WarEvent>();
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      try {
        setShowOnboarding(window.localStorage.getItem('warmap-onboarding-dismissed') !== '1');
  ...
  return <main className="min-h-screen bg-stone-950 text-stone-100">
    ...
    <div className="mx-auto max-w-[1600px] p-4 md:p-7">
      ...
      {showOnboarding && <section className="onboarding-panel" aria-label="快速开始"><div><p className="eyebrow">快速开始</p><h2>用三步读懂这张历史地图</h2><div className="onboarding-steps"><span><b>1</b>拖动时间轴查看历史变化</span><span><b>2</b>点击地图节点查看战争详情</span><span><b>3</b>播放叙事章节浏览关键事件</span></div></div><button type="button" className="onboarding-dismiss" onClick={() => { setShowOnboarding(false); try { window.localStorage.setItem('warmap-onboarding-dismissed', '1'); } catch { /* storage may be unavailable */ } }}>知道了</button></section>}
