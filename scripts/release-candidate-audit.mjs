...
const landmarkCount = (files.landmarks.match(/id: 'landmark-/g) ?? []).length;
const landmarkFiles = Array.from(files.landmarks.matchAll(/file: '([^']+\.webp)'/g), (match) => match[1]);
const uniqueLandmarkFiles = [...new Set(landmarkFiles)];
const landmarkStats = await Promise.all(uniqueLandmarkFiles.map(async (file) => ({ file, size: (await stat(path.resolve('public/landmarks', file))).size })));
const landmarkBytes = landmarkStats.reduce((sum, asset) => sum + asset.size, 0);
ensure(landmarkCount >= 15, `landmark imagery must cover at least 15 events, got ${landmarkCount}`);
ensure(files.landmarks.includes('sourcePageUrl:') && files.landmarks.includes('originalFileUrl:') && files.landmarks.includes('licenseUrl:'), 'landmark imagery must retain original page, file and license provenance');
ensure(files.landmarks.includes("classification: 'battle-map'") && files.landmarks.includes("classification: 'later-artwork'"), 'landmark imagery must distinguish battle maps from later artwork');
ensure(!files.landmarks.includes('.svg'), 'retired editorial SVG landmark imagery must not be referenced');
ensure(landmarkStats.every((asset) => asset.size <= 150 * 1024), 'each local landmark image must remain at or below 150 KiB');
ensure(landmarkBytes <= 1.45 * 1024 * 1024, `landmark image batch exceeds 1.45 MiB (${landmarkBytes} bytes)`);
...
const report = { version: packageJson.version, packages: ['qin-han', 'han-three-kingdoms'], counts: { qinEvents, hanEvents, threeKingdomsEvents, hanPlaces, territories, threeKingdomsNarrativeReferences: narratives, landmarkEvents: landmarkCount, landmarkFiles: uniqueLandmarkFiles.length }, landmarkBytes, landmarkAssets: landmarkStats, modernReferenceGzipBytes: gzipSync(files.modern).byteLength, historicalGis: 'disabled-awaiting-license', staticIndexBytes: outputSize, status: 'passed' };
