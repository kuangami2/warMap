...
export function WarDetailDrawer({ war, onClose, onCompare }: { war?: WarEvent; onClose: () => void; onCompare?: (war: WarEvent) => void }) {
  ...
  return <>
    ...
    <section ref={drawerRef} className="panel detail-drawer" role="dialog" aria-modal="true" aria-labelledby="war-detail-title" tabIndex={-1}>
      ...
      <div className="detail-tags"><span>{kindLabel[war.kind]}</span><span>{war.confidence === 'high' ? '高可信度' : war.confidence === 'medium' ? '中可信度' : '低可信度'}</span><span>{war.scale}规模</span></div>
      ...
      <div className="detail-facts"><div><p className="eyebrow">参与方</p><p>{war.participants.map((participant) => participant.name).join(' · ')}</p></div><div><p className="eyebrow">地点</p><p>{war.locations.map((location) => `${location.name}（${location.modernName ?? '位置待考'}）`).join('；')}</p></div></div>
      {war.locations.some((location) => location.note) && <p className="mt-2 text-sm leading-6 text-stone-400">地望说明：{war.locations.map((location) => location.note).filter(Boolean).join('；')}</p>}
      ...
      <div className="mt-5"><p className="eyebrow">可信度与来源</p>{war.uncertaintyNote && <p className="mt-2 text-sm leading-6 text-amber-100/80">争议与边界：{war.uncertaintyNote}</p>}<ul className="mt-2 space-y-2 text-sm text-stone-400">{war.sources.map((source) => <li key={source.id ?? source.title}><strong className="text-stone-300">{source.title}</strong>{source.citation ? `：${source.citation}` : ''}{source.claim ? `（${source.claim}）` : ''}{source.note ? <span className="block">{source.note}</span> : null}</li>)}</ul></div>
      {onCompare && <button type="button" className="compare-action" onClick={() => onCompare(war)}>加入事件对比</button>}
    </section>
  </>;
}
