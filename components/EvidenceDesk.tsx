...
export function EvidenceDesk({ data }: { data: ScenarioData }) {
  ...
  useEffect(() => {
    if (!open) { if (restoreFocusRef.current) { restoreFocusRef.current = false; toggleRef.current?.focus(); } return; }
    searchRef.current?.focus();
    const onKeyDown = (event: KeyboardEvent) => { if (event.key === 'Escape') { event.preventDefault(); closeDesk(); } };
    window.addEventListener('keydown', onKeyDown);
    return () => window.removeEventListener('keydown', onKeyDown);
  }, [open]);

  function closeDesk() { restoreFocusRef.current = true; setOpen(false); }
  ...
  return <section id="evidence-desk" className="evidence-desk" aria-label="来源证据台">
    <button ref={toggleRef} type="button" className="evidence-desk-toggle" onClick={() => open ? closeDesk() : setOpen(true)} aria-expanded={open} aria-controls="evidence-desk-panel">来源证据台{collection.length ? ` · ${collection.length}` : ''}</button>
    {open && <div id="evidence-desk-panel" className="evidence-desk-panel">
      <div className="evidence-desk-heading"><div><p className="eyebrow">研究工具</p><h2>来源证据台</h2></div><button type="button" className="close-button" onClick={closeDesk} aria-label="关闭来源证据台">×</button></div>
      <div className="evidence-desk-controls"><input ref={searchRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索来源标题或引文" aria-label="搜索来源" /><select value={kind} onChange={(event) => setKind(event.target.value as typeof kind)} aria-label="来源类型"><option value="all">全部来源</option><option value="primary">一手史料</option><option value="atlas">历史地图集</option><option value="modern-study">现代研究</option></select><select value={confidence} onChange={(event) => setConfidence(event.target.value as typeof confidence)} aria-label="关联可信度"><option value="all">全部可信度</option><option value="high">高可信度</option><option value="medium">中可信度</option><option value="low">低可信度</option></select></div>
